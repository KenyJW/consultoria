...
CREATE TABLE IF NOT EXISTS recommendations (
    ...
    -- Controles (15 controles) - asociados a dominios opcionalmente
    INSERT INTO iso_controls (domain_id, code, title, description, status) VALUES
    ((SELECT id FROM iso_domains WHERE code='D1'), 'C1', 'Política de seguridad', 'Existencia y comunicación de la política de seguridad', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D1'), 'C2', 'Organización de la seguridad', 'Roles y responsabilidades de seguridad', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D2'), 'C3', 'Inventario de activos', 'Identificación y clasificación de activos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D2'), 'C4', 'Propiedad de activos', 'Responsables y propietarios de activos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D3'), 'C5', 'Control de accesos lógicos', 'Gestión de cuentas y accesos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D3'), 'C6', 'Gestión de privilegios', 'Asignación y revisión de privilegios', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D4'), 'C7', 'Criptografía', 'Uso adecuado de controles criptográficos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D4'), 'C8', 'Seguridad en comunicaciones', 'Protección de información en tránsito', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D5'), 'C9', 'Seguridad en operaciones', 'Procedimientos operativos y seguridad', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D5'), 'C10', 'Gestión de incidentes', 'Detección, respuesta y registro de incidentes', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D6'), 'C11', 'Control físico', 'Protección de accesos físicos y equipos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D6'), 'C12', 'Protección ambiental', 'Medidas ambientales y de infraestructura', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D7'), 'C13', 'Evaluación de riesgos', 'Procesos de identificación y tratamiento de riesgos', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D7'), 'C14', 'Cumplimiento legal', 'Cumplimiento de requisitos legales y contractuales', 'active'),
    ((SELECT id FROM iso_domains WHERE code='D7'), 'C15', 'Auditoría y revisión', 'Revisión periódica y auditorías internas', 'active');

    -- Preguntas: 5 por cada control => 75 preguntas (textos de ejemplo)
